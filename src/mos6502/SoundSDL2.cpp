    ...
    myVolume(100)
{
  ...
  desired.freq   = 44100;//myOSystem->settings().getInt("freq");
  desired.format = AUDIO_S16SYS;
  desired.channels = 2;
  desired.samples  = 1470; // 2frames //myOSystem->settings().getInt("fragsize");
...
void SoundSDL2::open()
{
  ...
  myVolume = 100; //myOSystem->settings().getInt("volume");
  setVolume(myVolume);
